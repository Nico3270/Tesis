...
def sensibilidad_pendiente(a_carril, a_berma, p_promedio,l_tramo,d_sentido,p_no_rebase,p_autos,p_buses,p_camiones,volumen,cap,ns):
    ...
    plt.axvline(p_promedio,color="k", ls="dotted")
    plt.axhline(ns,color="k", ls="dotted")
    plt.grid(True)
    plt.legend()
    ...
    plt.style.use("seaborn-dark-palette")
    plt.plot(pendiente, cap_pen ,color="red", label='Capacidad')
    plt.scatter(p_promedio,cap, label="Resultado obtenido")
    plt.axvline(p_promedio,color="k", ls="dotted")
    plt.axhline(cap,color="k", ls="dotted")
    plt.text(p_promedio, cap+30, (p_promedio, cap),bbox=dict(facecolor='red', alpha=0.6), linespacing=2.5, weight='roman')
    plt.grid(True)
    plt.legend()
    plt.xlabel('Pendiente (%)')
...
def sensiblidad_longitud(a_carril, a_berma, p_promedio,l_tramo,d_sentido,p_no_rebase,p_autos,p_buses,p_camiones,volumen,cap, ns):
    #Datos de entrada
    longitud = np.arange(0.51,4.9,0.1)
    ns_lon = []
    cap_lon = []
    ...
    plt.style.use("seaborn-dark-palette")
    plt.plot(longitud, cap_lon ,color="red", label='Capacidad')
    plt.scatter(l_tramo,cap, label="Resultado obtenido", )
    plt.axvline(l_tramo,color="k", ls="dotted")
    plt.axhline(cap,color="k", ls="dotted")
    plt.text(l_tramo, cap+20, (l_tramo, cap),bbox=dict(facecolor='red', alpha=0.6), linespacing=2.5, weight='roman')
    plt.legend()
    plt.grid(True)
...
def sensibilidad_berma(a_carril, a_berma, p_promedio,l_tramo,d_sentido,p_no_rebase,p_autos,p_buses,p_camiones,volumen, ns, cap):
    ...
    plt.style.use("seaborn-dark-palette")
    plt.plot(berma, cap_berma ,color="red", label='Capacidad')
    plt.scatter(a_berma,cap, label="Resultado obtenido")
    plt.axvline(a_berma,color="k", ls="dotted")
    plt.axhline(cap,color="k", ls="dotted")
    plt.text(a_berma, cap+10, (a_berma, cap),bbox=dict(facecolor='red', alpha=0.6), linespacing=2.5, weight='roman')
    plt.legend()
    plt.grid(True)
...
def sensibilidad_carril(a_carril, a_berma, p_promedio,l_tramo,d_sentido,p_no_rebase,p_autos,p_buses,p_camiones,volumen, cap, ns):
    ...
    plt.style.use("seaborn-dark-palette")
    plt.plot(carril, cap_carril ,color="red", label='Capacidad')
    plt.scatter(a_carril,cap, label="Resultado obtenido", )
    plt.axvline(a_carril,color="k", ls="dotted")
    plt.axhline(cap,color="k", ls="dotted")
    plt.text(a_carril, cap+10, (a_carril, cap),bbox=dict(facecolor='red', alpha=0.6), linespacing=2.5, weight='roman')
    plt.legend()
    plt.grid(True)
...
def sensibilidad_camiones(a_carril, a_berma, p_promedio,l_tramo,d_sentido,p_no_rebase,p_autos,p_buses,p_camiones,volumen,ns,cap):
    ...
    plt.style.use("seaborn-dark-palette")
    plt.plot(camiones, cap_camion ,color="red", label='Capacidad')
    plt.scatter(p_camiones,cap, label="Resultado obtenido", )
    plt.axvline(p_camiones,color="k", ls="dotted")
    plt.axhline(cap,color="k", ls="dotted")
    plt.text(p_camiones, cap+20, (p_camiones, cap),bbox=dict(facecolor='red', alpha=0.6), linespacing=2.5, weight='roman')
    plt.legend()
    plt.grid(True)
